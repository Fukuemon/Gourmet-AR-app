...
import {
  ...
  PROPS_CATEGORY,
  PROPS_POST,
} from "src/types/Post/types"; // 必要なプロパティの型をインポート
...
export const getPostDetail = async (id: PROPS_POST) => {
  const res = await axios.get(
    `${process.env.NEXT_PUBLIC_RESTAPI_URL}api/post_detail/${id}/`,
    {
      headers: {
        "Content-Type": "application/json",
      },
...
export const fetchAsyncNewPost = createAsyncThunk(
  ...
  async (newPost: PROPS_NEWPOST) => {
    ...
    newPost.menu_item_photo &&
      uploadData.append(
        "menu_item_photo",
        newPost.menu_item_photo,
        newPost.menu_item_photo.name
      );
    newPost.menu_item_model &&
      uploadData.append(
        "menu_item_model",
        newPost.menu_item_model,
        newPost.menu_item_model.name
      );
...
//レストランの一覧を取得
export const fetchAsyncGetRestaurant = createAsyncThunk(
  "restaurant/get",
  async () => {
